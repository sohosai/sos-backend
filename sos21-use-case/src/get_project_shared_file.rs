...
use anyhow::Context;
use sos21_domain::context::{FileSharingRepository, Login, ProjectRepository};
...
where
    C: ProjectRepository + FileSharingRepository + Send + Sync,
{
...
mod tests {
    ...
    use crate::model::file_sharing::FileSharingId;
    use crate::model::project::ProjectId;
    use crate::{get_project_shared_file, UseCaseError};
    ...
    async fn test_general_others_project() {
        ...
        assert!(matches!(
            get_project_shared_file::run(&app, input).await,
            Err(UseCaseError::UseCase(
                get_project_shared_file::Error::ProjectNotFound
            ))
        ));
    }
    ...
    async fn test_general_others_owner() {
        ...
        assert!(matches!(
            get_project_shared_file::run(&app, input).await,
            Err(UseCaseError::UseCase(
                get_project_shared_file::Error::FileSharingNotFound
            ))
        ));
    }
    ...
    async fn test_general_owner_revoked() {
        ...
        assert!(matches!(
            get_project_shared_file::run(&app, input).await,
            Err(UseCaseError::UseCase(
                get_project_shared_file::Error::InvalidSharing
            ))
        ));
    }
