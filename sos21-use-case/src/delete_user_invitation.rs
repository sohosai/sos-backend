...
pub async fn run<C>(ctx: &Login<C>, id: UserInvitationId) -> UseCaseResult<(), Error>
where
    C: UserInvitationRepository + Send + Sync,
{
    ...
    ctx.delete_user_invitation(invitation.id())
        .await
        .context("Failed to delete user invitation")?;
...
mod tests {
    use crate::model::user_invitation::UserInvitationId;
    use crate::{delete_user_invitation, get_user_invitation, UseCaseError};
    ...
    async fn test_general() {
        ...
        assert!(matches!(
            delete_user_invitation::run(&app, UserInvitationId::from_entity(invitation.id())).await,
    ...
    async fn test_admin() {
        ...
        assert!(delete_user_invitation::run(&app, invitation_id)
            .await
            .is_ok());
        assert!(matches!(
            get_user_invitation::run(&app, invitation_id).await,
            Err(UseCaseError::UseCase(get_user_invitation::Error::NotFound))
        ));
    }
}
