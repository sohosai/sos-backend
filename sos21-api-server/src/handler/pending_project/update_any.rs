...
use sos21_use_case::update_any_pending_project;
...
impl From<update_any_pending_project::Error> for Error {
    fn from(err: update_any_pending_project::Error) -> Error {
        match err {
            update_any_pending_project::Error::NotFound => Error::PendingProjectNotFound,
            update_any_pending_project::Error::InsufficientPermissions => {
                Error::InsufficientPermissions
            }
            update_any_pending_project::Error::InvalidName => Error::InvalidField { field: "name" },
            update_any_pending_project::Error::InvalidKanaName => {
                Error::InvalidField { field: "kana_name" }
            }
            update_any_pending_project::Error::InvalidGroupName => Error::InvalidField {
                field: "group_name",
            },
            update_any_pending_project::Error::InvalidKanaGroupName => Error::InvalidField {
                field: "kana_group_name",
            },
            update_any_pending_project::Error::InvalidDescription => Error::InvalidField {
                field: "description",
            },
            update_any_pending_project::Error::DuplicatedAttributes => {
...
pub async fn handler(ctx: Login<Context>, request: Request) -> HandlerResult<Response, Error> {
    ...
    let pending_project = update_any_pending_project::run(&ctx, input).await?;
