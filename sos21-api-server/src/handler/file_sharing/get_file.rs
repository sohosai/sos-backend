...
use sos21_use_case::get_shared_file_object;
use warp::{
...
impl From<get_shared_file_object::Error> for Error {
    fn from(err: get_shared_file_object::Error) -> Error {
        match err {
            get_shared_file_object::Error::NotFound => Error::FileSharingNotFound,
            get_shared_file_object::Error::InvalidSharing => Error::InvalidFileSharing,
        }
    }
}
...
pub async fn handler(
    ...
    let file_object = get_shared_file_object::run(&ctx, request.sharing_id.into_use_case()).await?;
