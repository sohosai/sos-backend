...
use sos21_use_case::get_project_shared_file_object;
use warp::{
...
impl From<get_project_shared_file_object::Error> for Error {
    fn from(err: get_project_shared_file_object::Error) -> Error {
        match err {
            get_project_shared_file_object::Error::FileSharingNotFound => {
                Error::FileSharingNotFound
            }
            get_project_shared_file_object::Error::ProjectNotFound => Error::ProjectNotFound,
            get_project_shared_file_object::Error::InvalidSharing => Error::InvalidFileSharing,
        }
    }
}
...
pub async fn handler(
    ...
    let file_object = get_project_shared_file_object::run(&ctx, input).await?;
